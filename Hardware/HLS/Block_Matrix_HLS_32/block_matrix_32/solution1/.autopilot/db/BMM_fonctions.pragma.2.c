...
_ssdm_op_SpecInterface(out_mC, "m_axi", 0, 0, "", 0, 0, "OUTPUT", "slave", "", 16, 16, 16, 16, "", "");
 ...
 int i,j,k,ii,jj,kk;
    ...
    type mB[32][32];
    type mC[32][32];
    multiply_block_label0:for (i = 0; i < 32 ; i++)
     memcpy(&mA[i],&in_mA[i],32*sizeof(type));
    multiply_block_label1:for (i = 0; i < 32 ; i++)
        memcpy(&mB[i],&in_mB[i],32*sizeof(type));
    multiply_block_label2:for (i = 0; i < 32 ; i++)
        memcpy(&mC[i],&out_mC[i],32*sizeof(type));




i_loop:for(i=0; i<32; i+=4){
j_loop:for(j=0; j<32; j+=4){
k_loop:for(k=0; k<32; k+=4){
...
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 ...
 for (i = 0; i < 32 ; i++)
_ssdm_Unroll(0,0,0, "");
memcpy(&out_mC[i],&mC[i],32*sizeof(type));
