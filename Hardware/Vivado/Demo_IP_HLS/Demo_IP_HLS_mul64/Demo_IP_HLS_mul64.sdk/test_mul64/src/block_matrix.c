...
void init_multiply_block_ip(XMultiply_block_64* mb,XMultiply_block_64_Config* mb_c){
	...
	//int status=XMultiply_block_64_Initialize(mb,id);
...
void multiply_block_hw_call(XMultiply_block_64* mb_p,type* mA, type* mB, type* result){
	...
	//printf("in out set:\n");
	//printf("ma:%X=>%X,mb:%X=>%X,mc:%X=>%X\n",(u32)mA,XMultiply_block_64_Get_in_mA(mb_p),(u32)mB,XMultiply_block_64_Get_in_mB(mb_p),(u32)result,XMultiply_block_64_Get_out_mC(mb_p));
	//printf("waiting ready\n");
	while(!XMultiply_block_64_IsReady(mb_p));
	//printf("multiply is ready\n");
	XMultiply_block_64_DisableAutoRestart(mb_p);
...
{
	XMultiply_block_64 MB;
	XMultiply_block_64_Config mb_c ={0,XPAR_MULTIPLY_BLOCK_64_0_S_AXI_CONTROL_BUS_BASEADDR};
