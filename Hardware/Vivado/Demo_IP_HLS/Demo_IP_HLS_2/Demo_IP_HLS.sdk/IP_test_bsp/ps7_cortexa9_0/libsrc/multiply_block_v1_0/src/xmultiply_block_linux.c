...
typedef struct {
    ...
    XMultiply_block_uio_map maps[ MAX_UIO_MAPS ];
} XMultiply_block_uio_info;

/***************** Variable Definitions **************************************/
static XMultiply_block_uio_info uio_info;
...
static int uio_info_read_name(XMultiply_block_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMultiply_block_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMultiply_block_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMultiply_block_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMultiply_block_Initialize(XMultiply_block *InstancePtr, const char* InstanceName) {
	XMultiply_block_uio_info *InfoPtr = &uio_info;
